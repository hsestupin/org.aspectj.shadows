...
public class BatchAnnotationProcessorManager extends BaseAnnotationProcessorManager {
    ...
    public ProcessorInfo discoverNextProcessor() {
        FileWriter log = null;
        try {
            log = new FileWriter(new File("C:/serviceLoader.txt"));
            log.write("LOGGING ON");
            log.write("_setProcessors:" + _setProcessors + '\n');
            ...
            log.write("_commandLineProcessors:" + _commandLineProcessors + '\n');
            if (null != _commandLineProcessors) {
            ...
            log.write("_serviceLoader = " + _serviceLoader + '\n');
            log.write("_procLoader: " + _procLoader + '\n');
            log.write("Start process logging...\n");
            String resPath = "META-INF/services/" + Processor.class.getName();
            log.write("Services path:" + resPath + "\n");
            Enumeration<URL> resources = _procLoader.getResources(resPath);
            log.write("resources:" + resources + '\n');
            if (resources != null) {
                while (resources.hasMoreElements()) {
                    URL url = resources.nextElement();
                    log.write("resource:" + url + '\n');
                    log.write("Trying to parse...\n");
                    parse(Processor.class, url, log);
                }
            }
    ...
    private Iterator<String> parse(Class<?> service, URL u, FileWriter log)
        ...
        try {
            ...
            int lc = 1;
            while ((lc = parseLine(r, lc, names, log)) >= 0);
        } catch (IOException x) {
        ...
        log.write("name.size = " + names.size() + '\n');
        return names.iterator();
    }

    private int parseLine(BufferedReader r, int lc,
                          List<String> names, FileWriter log)
            throws IOException, ServiceConfigurationError
    {
        ...
        if (n != 0) {
            ...
            log.write("add new name = " + ln + '\n');
            if (!names.contains(ln))
                names.add(ln);
        }
        log.write("returning lc " + lc + '\n');
        return lc + 1;
    }
